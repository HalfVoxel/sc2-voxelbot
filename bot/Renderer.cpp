...
void ImageRGB(const char* bytes, int width, int height, int off_x, int off_y, int scale) {
    assert(renderer_);
    assert(window_);
    ...
    SDL_RenderCopyEx(renderer_, texture, NULL, &dstRect, 0, NULL, SDL_FLIP_VERTICAL);
