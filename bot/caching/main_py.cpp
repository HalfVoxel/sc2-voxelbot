...
void printMappings() {
    ...
    output << "Unit = namedtuple('Unit', ['name', 'is_army', 'type_ids', 'race', 'food_delta', 'mineral_cost', 'vespene_cost'])" << endl;
    ...
    for (int race = 0; race < 3; race++) {
        ...
        for (int j = 0; j < units.size(); j++) {
            ...
            float minerals = data.mineral_cost;
            float vespene = data.vespene_cost;
            if (hasBeen(us[0]).size() > 1 && !isAddon(us[0])) {
                auto& previous = getUnitData(hasBeen(us[0])[1]);
                minerals -= previous.mineral_cost;
                vespene -= previous.vespene_cost;
            }
